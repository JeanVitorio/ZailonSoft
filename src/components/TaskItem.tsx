    ...
    if (timeStatus.status === 'overdue') {
      return (
        <span className="flex items-center gap-0.5 text-[10px] font-bold text-destructive">
          ...
          {formatMinutes(timeStatus.minutesDiff)} atrasada
        </span>
      );
    }
    ...
    if (timeStatus.status === 'upcoming') {
      return (
        <span className="flex items-center gap-0.5 text-[10px] font-bold text-muted-foreground">
          ...
          em {formatMinutes(timeStatus.minutesDiff)}
        </span>
      );
    }

    if (timeStatus.status === 'now') {
      return (
        <span className="text-[10px] font-bold text-cta animate-pulse">
          AGORA
        </span>
      );
    }
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-2 mt-1.5">
          <span
            className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
              difficultyColors[task.dificuldade]
            }`}
          >
      ...
      <div className="flex flex-col items-center shrink-0 gap-1">
        <span className="text-xs font-extrabold text-cta">
          +{xp} XP
        </span>
