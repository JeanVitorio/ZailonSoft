...
const MainLayout = () => {
  ...
  const hasMismatch = isCustomHost && isLoggedIn && !loading && !hasTenantAccess(tenant?.id, lojaInfo?.id);

  React.useEffect(() => {
    if (!hasMismatch || mismatchHandled.current) return;
  ...
  if (!isLoggedIn) {
    return <Navigate to={isCustomHost ? '/admin/login' : '/login'} replace />;
  }
  ...
  if (!isActive) {
    ...
    if (status === 'past_due' || status === 'unpaid') {
      return <Navigate to={isCustomHost ? '/admin/inadimplente' : '/inadimplente'} replace />;
    }
    return <Navigate to={isCustomHost ? '/admin/assinar' : '/assinar'} replace />;
  }

  // If user is trying to access another store's slug, block
  if (!isCustomHost && urlSlug && lojaSlug && urlSlug !== lojaSlug) {
    return <Navigate to={`/${lojaSlug}/dashboard`} replace />;
  }

  const basePath = isCustomHost ? '/admin' : `/${lojaSlug || urlSlug}`;
