...
export const LeadForm: React.FC<LeadFormProps> = ({ isOpen, onClose, vehicle }) => {
  ...
  const lojaSlug = tenant?.slug || params.lojaSlug || authSlug || '';
  const resolvedLojaId = tenant?.id || lojaId;
  ...
  const handleSubmit = async () => {
    ...
    try {
      let cnhUrl = '';
      if (formData.cnhFile) {
        cnhUrl = await uploadCnhPublic(formData.cnhFile, lojaSlug || resolvedLojaId || 'shared');
      }
      ...
      await submitLead({
        loja_slug: tenant ? undefined : lojaSlug || undefined,
        loja_id: tenant?.id || (!lojaSlug ? resolvedLojaId || undefined : undefined),
        name: formData.name,
      ...
      case 'welcome':
        return (
          <div className="flex flex-col items-center text-center py-6 md:py-8">
            ...
            <h3 className="text-xl md:text-2xl font-bold text-white mb-3">Oi! Eu sou o Falcon</h3>
