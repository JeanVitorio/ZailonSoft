        ...
        <div className="fixed bottom-0 right-0 p-6 space-y-2 z-[100]">
            {toasts.map(toast => (
                <div key={toast.id} className={`p-4 rounded-md shadow-lg flex items-start gap-4 w-80 md:w-96 ${variantClasses[toast.variant]}`}>
                    ...
                    <button onClick={() => dispatch({ type: 'REMOVE_TOAST', id: toast.id })} className="p-1 rounded-full hover:bg-white/10">
                        <X className="h-4 w-4" />
                    </button>
                </div>
            ))}
...
function ClientCard({ client, onDelete, onViewDetails }) {
    ...
    return (
        <Card ref={setNodeRef} style={style} {...attributes} className="touch-none bg-background/80 backdrop-blur-sm shadow-md hover:shadow-lg w-full max-w-[calc(100%-1rem)] mx-auto sm:max-w-full box-border">
            <CardContent className="p-1.5 xs:p-2 space-y-2 relative">
                <div {...listeners} className="cursor-grab active:cursor-grabbing pr-8 xs:pr-10">
                    <h4 className="font-semibold text-[0.65rem] xs:text-xs truncate">{client.name || "Cliente sem nome"}</h4>
                    <p className="text-[0.65rem] xs:text-xs text-muted-foreground truncate">{interestedVehicle}</p>
                    <p className="text-[0.65rem] xs:text-xs text-muted-foreground truncate">{dealType}</p>
                </div>
...
function ClientDetailDialog({ client, isOpen, onOpenChange, updateMutation }) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className="w-full max-w-[95vw] md:max-w-4xl max-h-[95vh] p-0 flex flex-col">
                ...
                >
                    <div className="w-full overflow-x-hidden pb-4">
                        ...
                        >
                            {columns.length > 0 ? (
                                columns.map((column) => (
                                    <div key={column.id} className={`flex-shrink-0 min-w-0 ${isMobile ? 'w-full max-w-[calc(100vw-2.5rem)]' : 'w-72'} overflow-x-visible box-border`}>
                                        <SortableContext
                                            id={column.id}
                                        ...
                                        >
                                            <div className="flex flex-col gap-4 p-4 bg-muted/50 rounded-lg h-full">
                                                <div className="flex items-center justify-between">
                                                    <h3 className="font-semibold text-sm md:text-base truncate max-w-[80%]">{column.name}</h3>
