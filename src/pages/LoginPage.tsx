...
const LoginPage = () => {
  ...
  const { login, logout, isLoggedIn, lojaSlug, lojaInfo, loading: authLoading, subscription } = useAuth();
  const { tenant, isPlatformHost } = useTenant();
  const navigate = useNavigate();
  const { toast } = useToast();
  const mismatchHandled = React.useRef(false);
  const isCustomHost = !isPlatformHost;

  React.useEffect(() => {
    if (!authLoading && isLoggedIn) {
      if (isCustomHost && !hasTenantAccess(tenant?.id, lojaInfo?.id)) {
        if (!mismatchHandled.current) {
          mismatchHandled.current = true;
          toast({
            title: 'Acesso negado',
            description: 'Esta conta não pertence à loja deste domínio.',
            variant: 'destructive',
          });
          logout();
        }
      ...
      if (subscription?.status === 'active' && lojaSlug) {
        navigate(isCustomHost ? '/admin/dashboard' : `/${lojaSlug}/dashboard`, { replace: true });
      } else if (subscription?.status === 'past_due' || subscription?.status === 'unpaid') {
        navigate(isCustomHost ? '/admin/inadimplente' : '/inadimplente', { replace: true });
      } else {
        navigate(isCustomHost ? '/admin/assinar' : '/assinar', { replace: true });
      }
    }
  }, [authLoading, isCustomHost, isLoggedIn, lojaInfo?.id, lojaSlug, logout, navigate, subscription, tenant?.id, toast]);
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
          <div className="flex items-center justify-between mb-6">
            <Link to="/" className="inline-flex items-center gap-3">
              <span className="text-xl font-bold text-gradient">Falcon</span>
            </Link>
            <ThemeToggle />
          </div>

          {/* Toggle Login / Signup */}
          {!isCustomHost && <div className="flex p-1 rounded-2xl bg-muted/40 mb-6">
            <button
              type="button"
