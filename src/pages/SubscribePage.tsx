...
const SubscribePage = () => {
  ...
  useEffect(() => {
    if (subscription?.status === 'active' && lojaSlug) {
      navigate(isPlatformHost ? `/${lojaSlug}/dashboard` : '/admin/dashboard', { replace: true });
    }
  }, [subscription, lojaSlug, isPlatformHost, navigate]);
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('create-checkout', {
        ...
        body: {
          return_path: isPlatformHost ? '/assinar' : '/admin/assinar',
        },
      });
  ...
  const waUrl = `https://wa.me/${SUPPORT_PHONE}?text=${encodeURIComponent(
    `Olá! Tenho dúvidas sobre a assinatura do Falcon (${user?.email || ''}).`,
  )}`;
      ...
      >
        <div className="text-center mb-6">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-cyan-500/10 border border-cyan-500/20 mb-4">
            ...
            <span className="text-xs font-medium text-cyan-400">Plano Falcon</span>
          </div>
