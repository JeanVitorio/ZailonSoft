...
const PublicVehicleDetail = () => {
  ...
  useEffect(() => {
    const load = async () => {
      setIsLoading(true);
      try {
        const lojaData = lojaSlug
          ? await fetchLojaBySlug(lojaSlug)
          : tenant;
        if (!lojaData) throw new Error('Loja não encontrada.');
        if (isPlatformHost && lojaSlug && lojaData.dominio) {
          window.location.replace(`https://${lojaData.dominio}/veiculo/${id}`);
          return;
        }
        const car = await fetchCarDetails(id!, lojaData.id);
        setVehicle({
    ...
    load();
  }, [id, isPlatformHost, lojaSlug, tenant]);
