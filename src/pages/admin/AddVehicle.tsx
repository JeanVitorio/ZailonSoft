...
import { Car, FileText, Image, Check, ArrowLeft, ArrowRight, Upload, X, Trash2 } from 'lucide-react';
...
const AddVehicle = () => {
  ...
  const steps = [
    ...
    { icon: Image, label: 'Fotos', description: 'Imagens reais' },
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length + imageFiles.length > 10) {
      toast({ title: 'Limite de imagens', description: 'Máximo de 10 imagens por veículo', variant: 'destructive' });
      return;
    }
  ...
  const handleSubmit = async () => {
    if (!formData.name || !formData.brand || !formData.price) {
      toast({ title: 'Campos obrigatórios', description: 'Preencha nome, marca e preço', variant: 'destructive' });
      return;
    }
    if (imageFiles.length === 0) {
      toast({ title: 'Adicione fotos', description: 'Adicione pelo menos uma foto do veículo', variant: 'destructive' });
      return;
    }

    setIsSubmitting(true);
    try {
      await addVehicle(formData, imageFiles);
      toast({ title: 'Veículo cadastrado!', description: `${formData.name} foi adicionado ao catálogo` });
      navigate(`${adminBasePath}/catalogo`);
    } catch (err: any) {
  ...
  const canProceed = () => {
    switch (currentStep) {
      case 0: return formData.name && formData.brand && formData.year && formData.price > 0;
      case 1: return formData.description.length > 0;
      case 2: return imageFiles.length > 0;
      default: return true;
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
        ...
        {steps.map((step, index) => (
          <React.Fragment key={index}>
            <div className="flex flex-col items-center">
              ...
              <p className={`text-xs mt-2 hidden sm:block ${index <= currentStep ? 'text-white' : 'text-muted-foreground'}`}>{step.label}</p>
            </div>
            {index < steps.length - 1 && (
              <div className={`flex-1 h-0.5 mx-2 ${index < currentStep ? 'bg-emerald-500' : 'bg-white/10'}`} />
            )}
          </React.Fragment>
        ))}
      </div>
      ...
      {/* Form Content */}
      <motion.div key={currentStep} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="glass-card p-6 md:p-8 rounded-2xl">
        {currentStep === 0 && (
          <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                ...
                <Input value={formData.brand} onChange={(e) => handleInputChange('brand', e.target.value)} placeholder="Ex: Porsche" />
              </div>
              <div>
                <label className="block text-sm font-medium text-muted-foreground mb-2">Modelo *</label>
                <Input value={formData.model} onChange={(e) => handleInputChange('model', e.target.value)} placeholder="Ex: 911 Turbo S" />
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-muted-foreground mb-2">Nome Completo *</label>
              <Input value={formData.name} onChange={(e) => handleInputChange('name', e.target.value)} placeholder="Ex: Porsche 911 Turbo S" />
            </div>
            <div className="grid md:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-medium text-muted-foreground mb-2">Ano *</label>
                <Input type="number" value={formData.year} onChange={(e) => handleInputChange('year', parseInt(e.target.value))} />
              </div>
              <div>
                <label className="block text-sm font-medium text-muted-foreground mb-2">Preço (R$) *</label>
                <Input type="number" value={formData.price || ''} onChange={(e) => handleInputChange('price', parseInt(e.target.value) || 0)} placeholder="0" />
              </div>
              <div>
                <label className="block text-sm font-medium text-muted-foreground mb-2">Km Rodados</label>
                <Input type="number" value={formData.mileage || ''} onChange={(e) => handleInputChange('mileage', parseInt(e.target.value) || 0)} placeholder="0" />
              </div>
            </div>
            <div className="grid md:grid-cols-3 gap-4">
              <div>
              ...
              <div>
                ...
                <Input value={formData.color} onChange={(e) => handleInputChange('color', e.target.value)} placeholder="Ex: Preto" />
              </div>
            </div>
          </div>
        )}
          ...
          <div className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-medium text-muted-foreground mb-2">
                Descrição * <span className="text-xs ml-2">({formData.description.length}/1000)</span>
              </label>
            ...
            <div>
              ...
              <div className="flex gap-2 mb-3">
                <Input value={newFeature} onChange={(e) => setNewFeature(e.target.value)} placeholder="Ex: Teto Solar"
                  onKeyPress={(e) => e.key === 'Enter' && addFeature()} />
                <Button onClick={addFeature} variant="secondary">Adicionar</Button>
              </div>
        ...
        {currentStep === 2 && (
          <div className="space-y-6">
            <h2 className="text-xl font-semibold text-white mb-4">Fotos do Veículo</h2>
                ...
                {imagePreviews.map((preview, index) => (
                  <div key={index} className="relative aspect-video rounded-xl overflow-hidden group">
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      <button onClick={() => removeImage(index)} className="w-10 h-10 rounded-full bg-red-500/80 flex items-center justify-center text-white hover:bg-red-500">
          ...
          <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="aspect-video rounded-xl overflow-hidden">
                <img src={imagePreviews[0] || '/placeholder.svg'} alt="" className="w-full h-full object-cover" />
              </div>
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 pt-4">
                  {[
                    ['Quilometragem', `${formData.mileage.toLocaleString()} km`],
                    ['Câmbio', formData.transmission],
                ...
                <p className="text-xs text-muted-foreground">{imageFiles.length} foto(s) selecionada(s)</p>
              </div>
            </div>
      ...
      {/* Navigation */}
      <div className="flex items-center justify-between">
        <Button variant="secondary" onClick={() => currentStep > 0 ? setCurrentStep(currentStep - 1) : navigate(`${adminBasePath}/catalogo`)}>
        ...
        {currentStep < steps.length - 1 ? (
          ...
          <Button variant="premium" onClick={handleSubmit} disabled={isSubmitting}>
            {isSubmitting ? (
              ...
              <><Check className="w-4 h-4" /> Publicar Veículo</>
            )}
          </Button>
        )}
      </div>
