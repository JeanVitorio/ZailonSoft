...
const emptyStore: Store = {
  id: '', dominio: '', name: '', description: '', email: '', phone: '', whatsapp: '',
...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const refreshData = async () => {
    try {
      ...
      if (user && lojaId) {
        ...
        // Fetch clients
        try {
          const clientsData = await apiService.fetchClients(lojaId);
  ...
  const updateStore = async (updates: Partial<Store>) => {
    try {
      ...
      if (updates.dominio !== undefined) {
        supabaseUpdates.dominio = normalizeDomain(updates.dominio) || null;
      }
