...
import { isPlatformHostname } from '@/lib/domain';
...
export function AuthProvider({ children, queryClient }: { children: ReactNode; queryClient?: QueryClient }) {
  ...
  const loadLojaInfo = async (currentUserId: string | undefined, reqId: number) => {
    ...
    try {
      const { data, error } = await supabase
        .from('lojas')
        .select('id, slug, dominio, nome, logo_url')
        .eq('user_id', currentUserId)
        .maybeSingle();
      if (error) throw error;
      if (reqId !== reqIdRef.current) return;
  ...
  const logout = async () => {
    ...
    setSubscription(null);
    setLojaInfo(null);
    window.location.href = isPlatformHostname(window.location.hostname) ? '/login' : '/admin/login';
  };
