...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange(
      async (_event, session) => {
        ...
        // A lógica de busca da assinatura agora está unificada aqui
        if (currentUser) {
          ...
          // Garante que a assinatura seja limpa se não houver usuário
  ...
  // O useEffect para o Realtime continua o mesmo, garantindo atualizações instantâneas
  useEffect(() => {
    if (user) {
  ...
  const logout = async () => {
    ...
    // A navegação é simples, o listener acima vai cuidar de limpar todo o estado
    navigate('/login');
  };
