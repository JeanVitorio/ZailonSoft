...
export const fetchCarDetails = async (carId: string, lojaId: string): Promise<Car> => {
  const { data, error } = await supabase
    .from('cars')
    .select('*')
    .eq('id', carId)
...
export const fetchLojaByDomain = async (domain: string): Promise<LojaDetails> => {
  ...
  if (error) {
    console.error('Erro ao resolver loja pelo domínio:', error);
    throw new Error('Não foi possível consultar a loja deste domínio. Tente novamente em instantes.');
  }
  if (!data) {
    throw new Error('Este domínio ainda não está configurado para uma loja.');
  }
  return data as LojaDetails;
};
...
export const fetchClients = async (lojaId: string): Promise<Client[]> => {
  const { data, error } = await supabase
    .from('clients')
    .select('*')
    .eq('loja_id', lojaId);
...
export const updateStoreDetails = async ({
  ...
  const finalUpdates = {
    ...updates,
    ...(Object.prototype.hasOwnProperty.call(updates, 'dominio')
      ? { dominio: normalizeDomain(updates.dominio) || null }
      : {}),
  };
