  ...
  try {
    if (action === "list") {
      const { data: lojas, error } = await supabase
        .from("lojas")
        .select("id, slug, dominio, nome, email, telefone_principal, user_id, logo_url, created_at");
    ...
    if (action === "create") {
      ...
      const dominio = normalizeDomain(body.dominio) || null;
      if (dominio && !isValidDomain(dominio)) {
    ...
    if (action === "update_loja") {
      ...
      if (Object.prototype.hasOwnProperty.call(updates, "dominio")) {
        updates.dominio = normalizeDomain(updates.dominio) || null;
        if (updates.dominio && !isValidDomain(updates.dominio)) {
          return json({ error: "Domínio inválido" }, 400);
        }
      }
